defmodule MimeTypes do
  ...
  {ext2mime, mime2ext} =
    case :httpc.request('https://svn.apache.org/repos/asf/httpd/httpd/trunk/docs/conf/mime.types') do
      {:ok, {{_, 200, _}, _, r}} -> "#{r}"
  ...
  def bin2ext(<<0x04034B50::size(32)-little, _::binary>> = zip) do
    ...
    res =
      cond do
        Enum.all?(['[Content_Types].xml', 'word/styles.xml'], &(&1 in files)) ->
          ".docx"

        Enum.all?(['[Content_Types].xml', 'xl/styles.xml'], &(&1 in files)) ->
          ".xlsx"

        Enum.all?(['[Content_Types].xml', 'ppt/presProps.xml'], &(&1 in files)) ->
          ".pptx"
